...
We also use the same machinery to build a controlled support-deletion stress
test. Starting from a true or false source question, we delete one fact or rule
from its gold derivation chain and rerun the forward-chaining checker. A mutant
is retained only if the originally supported literal is no longer derivable and
the opposite literal is also not derivable. The retained examples are therefore
not counterexamples to the source claim; they are open-world cases where a
previously valid derivation has become unsupported.
